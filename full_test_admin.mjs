...
const testEmail = `admintest${Date.now()}@gmail.com`;
const testPassword = 'Test123456!';
const testBusinessName = 'Admin Test Business';
const testPhone = '07712345678';
const testGovernorate = 'Baghdad';
const testCategory = 'banks';
const testCity = 'Baghdad';

console.log('=== FULL RUNTIME TEST (ADMIN API) ===\n');

// Flow 1: Signup via admin API (bypasses rate limit)
async function flow1() {
  console.log('FLOW 1: Signup as Business Owner (via Admin API)');
  console.log('======================================================\n');
  ...
  try {
    ...
    if (profileError) {
      ...
      profileExists = true;
      console.log('✓ Profile exists (trigger might have worked)');
    }

    // Fallback: create profile manually (using service role to bypass RLS for test)
    if (!profileExists) {
      console.log('⚠️  Profile not found, creating fallback...');
      ...
      if (insertError) {
        ...
        return { pass: false, error: insertError.message };
      }
      console.log('✓ Profile created via fallback');
    }

    // Verify profile (using service role to bypass RLS for test)
    ...
    if (!finalProfile) {
      ...
      return { pass: false, error: 'Profile not found after creation' };
    }
